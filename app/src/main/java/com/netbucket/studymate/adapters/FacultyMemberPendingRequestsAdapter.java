...
public class FacultyMemberPendingRequestsAdapter extends FirestoreRecyclerAdapter<FacultyMember, FacultyMemberPendingRequestsAdapter.PendingRequestsHolder> {
    private final FirebaseFirestore mStore = FirebaseFirestore.getInstance();

    public FacultyMemberPendingRequestsAdapter(@NonNull FirestoreRecyclerOptions<FacultyMember> options) {
        super(options);
    }

    @Override
    protected void onBindViewHolder(@NonNull PendingRequestsHolder holder, int position, @NonNull FacultyMember model) {
