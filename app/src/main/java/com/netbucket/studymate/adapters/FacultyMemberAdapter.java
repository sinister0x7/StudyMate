...
public class FacultyMemberAdapter extends FirestoreRecyclerAdapter<FacultyMember, FacultyMemberAdapter.FacultyMemberHolder> {
    private final FirebaseFirestore mStore = FirebaseFirestore.getInstance();

    public FacultyMemberAdapter(@NonNull FirestoreRecyclerOptions<FacultyMember> options) {
        super(options);
    }

    @Override
    protected void onBindViewHolder(@NonNull final FacultyMemberHolder holder, final int position, @NonNull final FacultyMember model) {
        ...
        holder.constraintLayoutItem.setOnClickListener(v -> {
            ...
            View bottomSheetView = LayoutInflater.from(holder.constraintLayoutItem.getContext()).inflate(R.layout.bottom_sheet_member_options, holder.itemView.findViewById(R.id.linearLayout_bottom_sheet_container));

            LinearLayout grantEditProfileAccessLayout = bottomSheetView.findViewById(R.id.linearLayout_grant_profile_edit_access);
            LinearLayout revokeEditProfileAccessLayout = bottomSheetView.findViewById(R.id.linearLayout_revoke_profile_edit_access);

            if (model.getProfileEditAccess().equals("granted")) {
                grantEditProfileAccessLayout.setVisibility(View.GONE);
                revokeEditProfileAccessLayout.setVisibility(View.VISIBLE);
            }
            ...
            bottomSheetView.findViewById(R.id.linearLayout_grant_profile_edit_access).setOnClickListener(v12 -> {
                Map<String, Object> profileEditAccess = new HashMap<>();
                profileEditAccess.put("profileEditAccess", "granted");
                ...
                getSnapshots()
                        ...
                        .getReference()
                        .update(profileEditAccess);

                Toasty.success(v.getContext(), "Profile edit access granted!", Toast.LENGTH_SHORT, true).show();

            });

            bottomSheetView.findViewById(R.id.linearLayout_revoke_profile_edit_access).setOnClickListener(v12 -> {
                Map<String, Object> editAccess = new HashMap<>();
                editAccess.put("editAccess", "revoked");
                ...
                Toasty.success(v.getContext(), "Profile edit access revoked!", Toast.LENGTH_SHORT, true).show();

            });

            bottomSheetView.findViewById(R.id.linearLayout_suspend_member).setOnClickListener(v13 -> {
                Map<String, Object> userStatus = new HashMap<>();
                userStatus.put("userStatus", "suspended");
                ...
                mStore.collection("users")
                        ...
                        .addOnSuccessListener(aVoid -> {
                            ...
                            Toasty.success(v.getContext(), model.getFullName() + " suspended!", Toast.LENGTH_SHORT, true).show();
                        })
                        .addOnFailureListener(e -> Toasty.error(v.getContext(), "Failed to suspend " + model.getFullName() + ", Try again!", Toast.LENGTH_SHORT, true).show());
            });

            bottomSheetView.findViewById(R.id.linearLayout_delete_member).setOnClickListener(v14 -> {
                Map<String, Object> userStatus = new HashMap<>();
    ...
    @NonNull
    @Override
    public FacultyMemberHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_faculty_member, parent, false);
        return new FacultyMemberHolder(view);
    }

    static class FacultyMemberHolder extends RecyclerView.ViewHolder {
        ...
        public FacultyMemberHolder(View itemView) {
            super(itemView);
